...
维度,docker-compose.yaml (你之前敲的),first_bq_query_pipeline.py (你刚才改的)
扮演角色,地产开发商 / 硬件安装工,车间排班经理 / 生产线图纸
核心职责,负责在你的 Mac 里规划出：1. 哪里放数据库（Postgres）2. 哪里开网页端口（8080）3. 怎么把 Mac 硬盘和容器连起来（Volumes）,负责在具体的业务里规定：1. 这个任务叫什么名字（dag_id）2. 到底什么时候自动起爆（schedule_interval）3. 具体去谷歌云执行哪一句 SQL
修改频率,几个月动一次（除非要加新服务或改端口）,每天都在写（每来一个新指标，就建一个新 .py）


## 什么是幂等性（Idempotence）？
在最通俗的语境下，幂等性就是系统对“重复操作”的免疫力。

你可以把“不幂等”的操作想象成银行取款：你点一次“取 100 块”，账户就少 100；如果你因为网络卡顿手抖连点了 3 次，账户就会被扣掉 300 块。这就叫不幂等。
而“幂等”的操作就像是点亮卧室的开关：开关已经是“开”的状态了，你再去按 10 次、100 次“开”，灯依然是亮着的，不会因为你按了 100 次，灯的亮度就翻 100 倍，或者把灯烧掉。

### 在数据管道里：
- 不幂等（灾难）：任务失败了你点击重跑，它盲目地往数据库里又追加了一批数据。结果就是订单量翻倍，财务报表直接爆炸。
- 幂等（工业标准）：无论这个任务是因为报错自动重试，还是你手动点击重跑，执行 1 次和执行 100 次，数据仓库里最终沉淀的数据完全一模一样，没有任何一条重复，也没有任何缺失。

### 核心设计原则：如何做到幂等？
在构建任何数据流水线（DAG）时，要做到幂等，必须死守以下三大工业设计原则：

1. 唯一标识与状态卡位（Upsert / Unique Key）

如果是写入关系型数据库，必须为每一条数据设计一个“物理唯一键”（比如 order_id）。写入时，如果发现这个 ID 已经存在，就执行更新（Update）而不是插入（Insert）。

2. 物理分区与冷酷覆盖（Truncate & Overwrite）
- 如果是大数据集（如 BigQuery/Hive），更新单条记录代价极高。我们的做法是把数据按日期拆成一个个“抽屉”（分区）。
- 原则：重跑今天，就用今天的数据彻底覆盖并洗洗干净今天的抽屉，其他日期的抽屉动都不动。

3. 断绝动态系统时间（Deterministic Time）

- 代码里绝对禁止使用 datetime.now() 或 CURRENT_DATE。因为“现在”是一个永远在变的变量。
- 原则：必须使用 Airflow 传给你的固定历史时间锚点（Logical Date）。这样无论你在礼拜天重跑，还是礼拜一重跑，代码处理的永远是图纸上锁定的那一天的数据