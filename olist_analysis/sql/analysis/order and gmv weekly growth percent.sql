...
daily_lag AS (
    SELECT
        ...
        LAG(order_quantity,7) OVER(ORDER BY order_date) AS lastweek_quantity,
...
SELECT
    ...
    ROUND(
        (order_quantity - lastweek_quantity)::NUMERIC / NULLIF(lastweek_quantity, 0) * 100, 
        2
    ) AS order_weekly_growth_pct,
    ROUND(
        (gmv - lastweek_gmv)::NUMERIC / NULLIF(lastweek_gmv, 0) * 100, 
        2
    ) AS gmv_weekl_growth_pct
FROM daily_lag
ORDER BY order_date;
